...
if __name__ == '__main__':
    ...
    local_rcfile = os.path.join(magpath, '.magicrc')
    if os.path.isfile(local_rcfile):
        rcfile = local_rcfile

    gdspath = gds_dir_path
    project_gds = os.path.join(gdspath, project + '.gds')
    fill_gds = os.path.join(gdspath, project_with_id + '_fill_pattern.gds')
    seal_gds = os.path.join(gdspath, 'advSeal_6um_gen.gds')

    # Fail fast with explicit diagnostics before invoking magic.
    missing_gds = []
    for g in (project_gds, fill_gds, seal_gds):
        if not os.path.isfile(g):
            missing_gds.append(g)
    if missing_gds:
        print('Error: Missing required GDS input(s):')
        for g in missing_gds:
            print('  ' + g)
        sys.exit(1)
    ...
    with open(user_project_path + '/mag/compose_final.tcl', 'w') as ofile:
        ...
        # Compose using vendor placeholders only.  This avoids loading the
        # large MAG hierarchy (which is brittle across environments) and
        # forces stream-out from explicit GDS files.
        print('path search {}', file=ofile)
        print('load ' + project + ' -silent', file=ofile)
        print('property GDS_FILE ' + project_gds, file=ofile)
        ...
        print('box values 0 0 2 2', file=ofile)
        print('paint comment', file=ofile)
        print('property GDS_FILE ' + fill_gds, file=ofile)
        print('property GDS_START 0', file=ofile)
        print('property FIXED_BBOX "0 0 2 2"', file=ofile)
        ...
        print('load advSeal_6um_gen -silent', file=ofile)
        print('property GDS_FILE ' + seal_gds, file=ofile)
        print('property GDS_START 0', file=ofile)
    ...
    # Use full MAG views to match the integration flow used by "ship".
    myenv['MAGTYPE'] = 'mag'

    print('Building final GDS file ' + project_with_id + '.gds', flush=True)

    # Magic with Tcl<9 is known to be unstable when RLIMIT_NOFILE is very high.
    # Clamp soft limit to 1024 for deterministic file handling.
    try:
    ...
    if mproc.stderr:
        ...
        for line in mproc.stderr.splitlines():
            ...
            if line.startswith("No such file or directory"):
                ...
                if "Calma output error" in line or line.startswith("ERROR"):
                    had_magic_errors = True
